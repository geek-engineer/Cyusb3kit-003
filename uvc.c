...
{
//	CyU3PDebugPrint (4, "Enter CyFxUvcApplnDmaCallback!\n");
...
//            CyU3PDebugPrint (4, "DMA dmaBuffer.count:%d, dmaBuffer.size: %d!\n", dmaBuffer.count, dmaBuffer.size);
            /* Commit Buffer to USB*/
            status = CyU3PDmaMultiChannelCommitBuffer (chHandle, (dmaBuffer.count + CY_FX_UVC_MAX_HEADER), 0);

            if (status == CY_U3P_SUCCESS)
            {
//            	CyU3PDebugPrint (4, "DMA return: %d!\n", status);
#ifdef DEBUG_PRINT_FRAME_COUNT
                glDmaDone++;
#endif
            }
        ...
        )
{
     ...
     * APIs.
     */
	CyU3PDebugPrint (4, "CyFxGpifCB current state:%d!\n", currentState);
    switch (currentState)
    {
...
#define i2c_ADDR_WR 0xA8             /* Slave address used to write sensor registers. */
#define i2c_ADDR_RD 0xA9             /* Slave address used to read from sensor registers. */
